namespace Bond.RuntimeObject.UnitTests
{
    ...
    using Bond.RuntimeObject;
    using NUnit.Framework;

    [TestFixture]
    public class RuntimeCloneTests
    {
        [Test]
        public void Clone_ReturnsCorrectClonedObjectForSimpleStructWithBonded()
        {
            var schema = Schema<SingleBondedStructField>.RuntimeSchema;

            var expected = new RuntimeObject();

            var child = new RuntimeObject();
            child.Properties["Name"] = "Test";

            expected.Properties["Child"] = new RuntimeBonded<RuntimeObject>(child, schema,
                schema.SchemaDef.structs[schema.SchemaDef.root.struct_def].fields[0].type);

            var target = new RuntimeCloner(schema, schema.SchemaDef.root);
            var actual = target.Clone<RuntimeObject>(expected);
            ...
            Assert.IsTrue(actual.Properties.ContainsKey("Child"));
            Assert.IsTrue(actual.Properties["Child"] is RuntimeBonded<RuntimeObject>);

            var bondedActualChild = (RuntimeBonded<RuntimeObject>)actual.Properties["Child"];
            var actualChild = ((IRuntimeBonded<RuntimeObject>)bondedActualChild).Deserialize();

            Assert.IsTrue(actualChild.Properties.ContainsKey("Name"));
            Assert.IsTrue(actualChild.Properties["Name"] is string);
            Assert.AreEqual("Test", (string)actualChild.Properties["Name"]);
        }

        [Test]
        public void Clone_ReturnsCorrectClonedObjectForStructWithBonded()
        {
